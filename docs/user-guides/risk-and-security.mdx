...
The system calculates the ratio between the target wallet's **actual execution price** and the current Oracle price:

> If the actual price paid by the target wallet exceeds the Oracle reference price by more than **3x (300%)**, the copy trade is automatically cancelled.
