...
export default async function copyTradeRoutes(fastify: FastifyInstance) {
    ...
    fastify.post<{ Body: CreateConfigBody }>('/config', { preHandler: requireAuth }, async (request, reply) => {
        ...
        console.log(`[CopyTrade] POST /config - Request from ${userId}`, {
            target: request.body.targetWallet,
            amount: request.body.buyAmountUsd
        });
        ...
        try {
            ...
            const config = await prisma.copyTradeConfig.create({
                data: {
                    userId: user.id,
                    targetWallet: normalizeAddress(targetWallet),
                    chainId,
                    buyAmountUsd,
                    maxSlippageBps,
                    minMarketCapUsd,
