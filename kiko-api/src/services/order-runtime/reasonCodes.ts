...
export function inferOrderReasonCode(input?: string | null): OrderReasonCode {
  ...
  if (message.includes('all rpc endpoints failed')) return 'rpc_uncertain';
  if (message.includes('not visible') || message.includes('broadcasted_unseen')) return 'pending_visibility';
