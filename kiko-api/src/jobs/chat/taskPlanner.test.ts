...
test("materializePlanCard upgrades structural planning into a user-visible runtime card", () => {
  ...
  assert.equal(plan.visibility, "visible");
  assert.equal(
    plan.summary,
    "I will gather the necessary evidence first, then answer from the real results.",
  );
...
test("buildTaskPlanningContext follows resolved search contract instead of raw canonical realtime flags", () => {
  ...
  assert.equal(planning.plan.visibility, "internal");
  assert.equal(
