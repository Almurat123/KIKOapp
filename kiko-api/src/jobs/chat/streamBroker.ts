...
//         are coalesced later in WebSocket/frontend handling. Runtime plans
//         later became user-visible again, so the broker preserves plan
//         visibility hints when merging warmup, structural, and model-filled
//         planner states. The hint must not be interpreted as a frontend hide
//         switch because old persisted cards still need to render after refresh.
...
// - Applied To: preserving PlanCard.visibility hints through runtime-plan merges
...
import {
  getTextMetrics,
  logChatStreamDebug,
} from "../../services/chatStreamDebug.js";
import {
...
export class ChatStreamBroker {
  ...
  private visibleArtifacts: Array<{
    kind: string;
    target: "assistant_message" | "dedicated_message";
  }> = [];
  private lastPersistMs = 0;
  private planCard: PlanCard | null = null;
  private assistantData: Record<string, any> = {};
  private readonly contentSuppressor =
    createLeadingInternalScaffoldSuppressor();
  ...
  start() {
    ...
    if (this.started) {
      logChatStreamDebug(
        ...
        "ChatStreamBroker: duplicate message_start suppressed",
        {
    ...
    logChatStreamDebug(
      LogCode.AI_ORCHESTRATOR,
      "ChatStreamBroker: message_start broadcast",
      {
        sessionId: this.params.sessionId,
        taskId: this.params.taskId,
        assistantMessageId: this.params.assistantMessageId,
  ...
  async pushText(text: string) {
    ...
    logChatStreamDebug(
      LogCode.AI_ORCHESTRATOR,
      "ChatStreamBroker: content chunk emitted",
      {
        sessionId: this.params.sessionId,
        taskId: this.params.taskId,
        assistantMessageId: this.params.assistantMessageId,
  ...
  async pushReasoning(text: string) {
    ...
    logChatStreamDebug(
      LogCode.AI_ORCHESTRATOR,
      "ChatStreamBroker: reasoning chunk emitted",
      {
        sessionId: this.params.sessionId,
        taskId: this.params.taskId,
        assistantMessageId: this.params.assistantMessageId,
  ...
  async blockContent(
    replacement: string,
    options?: { clearReasoning?: boolean },
  ) {
  ...
  async bootstrapRuntime(plan: PlanCard) {
    ...
    this.planCard.activity = [
      this.makeRuntimeEvent(
        "bootstrap",
        this.localeText("Plan created", "已创建任务流程"),
        {
          detail: {
            title: plan.title,
  ...
  async applyModelPlan(plan: PlanCard): Promise<PlanCard> {
    if (
      this.planCard?.activity?.some((event) =>
        [
          "tool_started",
          "tool_completed",
          "tool_failed",
    ...
    this.planCard.activity = this.appendRuntimeEvent(
      ...
      this.makeRuntimeEvent(
        "plan_created",
        this.localeText("Plan updated from model", "已用模型计划更新任务卡"),
      ),
    );
    await this.persistRuntimeState("streaming");
    return this.clonePlan(this.planCard);
  ...
  async noteProviderProgress(progress: {
    status?: string;
    toolBatch?: Record<string, any>;
  }) {
    const status = String(progress.status || "").trim();
    const toolBatch =
      progress.toolBatch && typeof progress.toolBatch === "object"
        ? progress.toolBatch
        : undefined;
    const summary =
      status ||
      (typeof toolBatch?.phase === "string"
        ? `Provider progress: ${toolBatch.phase}`
        : "");
  ...
  async markPlanStepStarted(call: OrchestratorToolCall, step?: PlanStep) {
    ...
    next.activity = this.appendRuntimeEvent(
      next.activity,
      this.makeRuntimeEvent("tool_started", execution.summary, {
  ...
  async markAnswerStarted(step?: PlanStep) {
    ...
    next.activity = this.appendRuntimeEvent(
      ...
      this.makeRuntimeEvent(
        "answer_started",
        this.localeText("Writing the answer", "正在生成回答"),
        {
          stepId: targetStep?.id,
          status: "in_progress",
        },
  ...
  async recordToolResult(result: OrchestratorToolResult) {
    ...
    try {
        ...
        if (result.result !== undefined) {
          if (
            ...
            return {
              ...
              error:
                (result.result as Record<string, any>).error || result.error,
            };
          }
      ...
      const nextPolymarketSelection = extractPolymarketSelectionState(
        result.name,
        normalizedResult,
      );
      ...
      logger.warn(
        LogCode.DB_TRANSACTION_FAILED,
        "ChatStreamBroker: failed to record tool result",
        {
          messageId: this.params.assistantMessageId,
  ...
  async complete(overrides?: { content?: string }) {
    if (typeof overrides?.content === "string") {
      ...
      if (bufferedTail) {
        ...
        logChatStreamDebug(
          LogCode.AI_ORCHESTRATOR,
          "ChatStreamBroker: content suppressor tail emitted",
          {
            sessionId: this.params.sessionId,
            taskId: this.params.taskId,
            assistantMessageId: this.params.assistantMessageId,
    ...
    if (this.params.userId) {
      logChatStreamDebug(
        LogCode.AI_ORCHESTRATOR,
        "ChatStreamBroker: message_complete broadcast",
        {
          sessionId: this.params.sessionId,
          taskId: this.params.taskId,
          assistantMessageId: this.params.assistantMessageId,
  ...
  async fail(
    message: string,
    options?: { replaceContent?: boolean; clearReasoning?: boolean },
  ) {
    ...
    try {
      ...
      logger.warn(
        LogCode.DB_TRANSACTION_FAILED,
        "ChatStreamBroker: failed to persist error state for assistant message",
        {
          messageId: this.params.assistantMessageId,
          error: error?.message || String(error),
        },
  ...
  private async persistToolSideEffects(result: OrchestratorToolResult) {
    ...
    if (txMessageId && this.params.userId) {
      try {
        ...
        const txData =
          txMessage?.data || clientAction.data || clientAction.payload || {};
        const actionType =
          clientAction.type === "show_cross_chain_status_card"
        ...
        logger.warn(
          LogCode.WS_ERROR,
          "ChatStreamBroker: failed to rebroadcast dedicated transaction card",
          {
            assistantMessageId: this.params.assistantMessageId,
            txMessageId,
            error: error?.message || String(error),
  ...
  async emitProviderClientAction(clientAction: any) {
    if (!clientAction || typeof clientAction !== "object" || !clientAction.type)
      return;
  ...
  private async persistClientAction(
    clientAction: any,
    rawResult: any,
  ): Promise<string | null> {
    if (this.params.userId) {
    ...
    else if (clientAction.type === "show_strategy_card")
      dbMessageType = "strategy-card";
    else if (clientAction.type === "show_token_card")
      dbMessageType = "token-card";
    else if (clientAction.type === "show_polymarket_card")
      dbMessageType = "polymarket-embed";
    else if (
      clientAction.type === "show_transaction_status_card" ||
      clientAction.type === "show_cross_chain_status_card"
    ) {
  ...
  private async markPlanStepFinished(result: OrchestratorToolResult) {
    ...
    const inProgressIndex =
      matchingIndex >= 0
        ...
        : findLastIndex(
            executions,
            (item) =>
              item.toolName === result.name && item.status === "in_progress",
          );
    ...
    next.currentStepId = next.steps.find(
  ...
  private async markPlanFailed(message: string) {
    ...
    const now = new Date().toISOString();
    const active =
      next.steps.find((step) => step.status === "in_progress") ||
    ...
    next.activity = this.appendRuntimeEvent(
      next.activity,
      this.makeRuntimeEvent("runtime_error", message, {
        detail: { error: message },
        status: "failed",
      }),
    );
  ...
  private resolveStepForTool(
    ...
    return (
      plan.steps.find(
        (step) =>
          step.status === "in_progress" &&
          (step.preferredTools || []).includes(toolName),
      ) ||
      plan.steps.find(
        (step) =>
          step.status === "pending" &&
  ...
  private resolveStepForResult(
    plan: PlanCard,
    result: OrchestratorToolResult,
  ): PlanStep | undefined {
    return (
      plan.steps.find((step) =>
        (step.executions || []).some((item) => item.id === result.id),
  ...
  private broadcastAgentRuntime() {
    ...
    const latestEvent =
      this.planCard.activity?.[this.planCard.activity.length - 1] || null;
  ...
  private mergePlanCard(current: PlanCard, incoming: PlanCard): PlanCard {
    ...
    const preservedSteps = current.steps.filter(
      (step) =>
        !incoming.steps.some((incomingStep) => incomingStep.id === step.id),
    );
  ...
  private appendRuntimeEvent(
    activity: AgentRuntimeEvent[] | undefined,
  ...
  private registerVisibleArtifact(
    kind: string,
    target: "assistant_message" | "dedicated_message",
  ) {
    if (!kind) return;
    const exists = this.visibleArtifacts.some(
      (artifact) => artifact.kind === kind && artifact.target === target,
    );
    if (exists) return;
...
function extractRenderContract(result: any): any | null {
  ...
  return renderContract && typeof renderContract === "object"
    ? renderContract
    : null;
}
...
function mergeProviderNativeEvidence(
  ...
  if (duplicate) {
    duplicate.results = mergeProviderNativeResults(
      duplicate.results || [],
      incoming.results || [],
    );
...
function mergeProviderNativeResults(
  existing: ProviderNativeEvidenceSnapshot["results"],
  incoming: ProviderNativeEvidenceSnapshot["results"],
) {
  const seen = new Set(
    existing.map(
      (item) =>
        `${item.sourceType}:${item.url || ""}:${item.snippet || ""}:${item.title || ""}`,
    ),
  );
  const merged = [...existing];
...
function describeToolAction(
  ...
  const labels: Record<string, { en: string; zh: string }> = {
    external_web_search: {
      en: "check web and social context",
      zh: "获取网页与社交上下文",
    },
    ...
    get_economic_calendar: {
      en: "fetch economic calendar",
      zh: "获取经济日历",
    },
    get_polymarket_market_overview: {
      en: "fetch Polymarket overview",
      zh: "获取 Polymarket 概览",
    },
    get_polymarket_coin_updown_markets: {
      en: "find 5-minute coin markets",
      zh: "查找 5 分钟代币盘口",
    },
  };
...
export function shouldCollapseRuntimeResult(
  result: OrchestratorToolResult,
...
export function mergeCollapsedExecutionDetail(
  existingDetail: any,
  result: OrchestratorToolResult,
) {
  const source = String(result.metadata?.source || "").trim();
  const current =
    existingDetail && typeof existingDetail === "object"
      ...
      : {};
  const collapsed =
    current.collapsed && typeof current.collapsed === "object"
      ? { ...current.collapsed }
      : {};
  const countKey =
    source === "tool_budget_guard" ? "budgetGuardCount" : "repeatCacheCount";
  const noteKey =
    source === "tool_budget_guard" ? "lastBudgetGuard" : "lastRepeatCache";
...
export function mergeOrchestratorUsage(
  ...
  const promptTokens =
    Number(existing?.prompt_tokens || 0) + Number(incoming?.prompt_tokens || 0);
  const completionTokens =
    Number(existing?.completion_tokens || 0) +
    Number(incoming?.completion_tokens || 0);
  const reasoningTokens =
    getUsageReasoningTokens(existing) + getUsageReasoningTokens(incoming);
  const reportedTotal =
    Number(existing?.total_tokens || 0) + Number(incoming?.total_tokens || 0);
  const computedTotal = promptTokens + completionTokens + reasoningTokens;
  const costInUsdTicks =
    Number(existing?.cost_in_usd_ticks || 0) +
    Number(incoming?.cost_in_usd_ticks || 0);
  const promptCacheHitTokens =
    Number(existing?.prompt_cache_hit_tokens || 0) +
    Number(incoming?.prompt_cache_hit_tokens || 0);
  const promptCacheMissTokens =
    Number(existing?.prompt_cache_miss_tokens || 0) +
    Number(incoming?.prompt_cache_miss_tokens || 0);
  const promptTokensDetails = mergeUsageDetails(
    existing?.prompt_tokens_details,
    incoming?.prompt_tokens_details,
  );
  const completionTokensDetails = mergeUsageDetails(
    existing?.completion_tokens_details,
    incoming?.completion_tokens_details,
  );
  ...
  return {
    ...
    prompt_cache_hit_tokens:
      promptCacheHitTokens > 0 ? promptCacheHitTokens : undefined,
    prompt_cache_miss_tokens:
      promptCacheMissTokens > 0 ? promptCacheMissTokens : undefined,
    prompt_tokens_details: promptTokensDetails,
    completion_tokens_details: completionTokensDetails,
  };
}

function getUsageReasoningTokens(
  usage: OrchestratorUsage | null | undefined,
): number {
  if (!usage) return 0;
  return Number(
...
function compactDetail(value: any, depth = 0): any {
  ...
  if (Array.isArray(value)) {
    ...
    return value.length > 8
      ? [...limited, `... ${value.length - 8} more item(s)`]
      : limited;
  }
  if (depth >= 2) {
    ...
    return Object.keys(value).length > 10
      ? {
          ...summary,
          _truncated: `${Object.keys(value).length - 10} more field(s)`,
        }
      : summary;
  }
