    ...
    return [
        {
            ...
            content: [
                ...
                'Use domain=token with intent=clanker_deploy as the legacy token-deploy envelope for requests to deploy, launch, create, mint, 发币, 发行代币, 上线代币, or 创建代币. Runtime provider mapping is chain-based: Base (8453) uses Clanker, BNB Chain / BSC (56) uses Four.meme, and other explicit chains are unsupported for deploy in this system. Prefer task_mode=execute for launch preparation and task_mode=confirm only when the latest user turn explicitly confirms a previously prepared deploy payload. Do not ask for supply or decimals for these launch flows.',
